            ...
            if(matches){
                ...
                //Check neighbors
                 ...
                 if((best_right.line>=Img.Macroblocks[i].LineStart && best_right.line<=Img.Macroblocks[i].LineEnd)
               	 && (best_right.column>=Img.Macroblocks[i].ColumnStart && best_right.column<=Img.Macroblocks[i].ColumnEnd)){
                	if(pass1.raster[best_right_id - 1] <= 0){
                      	if(super_level.raster[best_right_id-1]==super_level.raster[best_id-1]){
			    		    //Calculate heterogeneity
                      		best_right_h = 0.0;
                 ...
                 if((best_left.line>=Img.Macroblocks[i].LineStart && best_left.line<=Img.Macroblocks[i].LineEnd)
               	 && (best_left.column>=Img.Macroblocks[i].ColumnStart && best_left.column<=Img.Macroblocks[i].ColumnEnd)){
                	if(pass1.raster[best_left_id - 1] <= 0){
					    if(super_level.raster[best_left_id-1]==super_level.raster[best_id-1]){
						    //Calculate heterogeneity
                      		best_left_h = 0.0;
...
int Slow_Mode::secondpass(Level& pass2, ERS_Image& Img, MSEG_Init& mseg, Level& previous_pass){
    ...
    pass2.Texture = previous_pass.Texture;
...
int Slow_Mode::secondpass(Level& pass2, ERS_Image& Img, MSEG_Init& mseg, Level& previous_pass, Level& super_level){
        ...
        //Check for match
	 	if(matches){
	 	    ...
	 	    for(int i=0; i<previous_pass.Objects[best_id].Neighbors.size(); i++){
	 	        ...
	 	        if(!previous_pass.Objects[near_id].merged){//if Neighbor not merged
                    //cout << "Neighbor not merged, inside if..." << endl;//Debug mode
                    if((super_level.raster[D1(Img.Lines, Img.Columns, previous_pass.Objects[near_id].Boundary[0].Line, previous_pass.Objects[near_id].Boundary[0].ColumnStart)])==(super_level.raster[D1(Img.Lines, Img.Columns, previous_pass.Objects[best_id].Boundary[0].Line, previous_pass.Objects[best_id].Boundary[0].ColumnStart)])){
						if (!previous_pass.Objects[best_id].virtual_merged){//if neighbor merging heterogeneity not calculated
                            //Virtual merge into temp object
                        	tmp.clear();
                        	tmp.merge(previous_pass.Objects[best_id], previous_pass.Objects[near_id]);
...
int Slow_Mode::nthpass(Level& passn, ERS_Image& Img, Texture_Image& TexImg, float level_ID, MSEG_Init& mseg, MSEG_Param& Param, Level& sub_level){
    ...
    while (!mseg.PriorityList.empty()){
        ...
        //If not calculated, calculate object's texture features //added
		if(sub_level.Objects[cur_id].SDM.size() <= 1){
		    sub_level.Objects[cur_id].BuildSDM(TexImg);
		}
		if(sub_level.Objects[cur_id].ASM.size() <= 1){
		    sub_level.Objects[cur_id].CalculateASM(TexImg);
		}
		...
		for(int i=0; i<sub_level.Objects[cur_id].Neighbors.size(); i++){
		    ...
		    //If not calculated, calculate object's texture features //added
            if(sub_level.Objects[near_id].SDM.size() <= 1){
                sub_level.Objects[near_id].BuildSDM(TexImg);
            }
            if(sub_level.Objects[near_id].ASM.size() <= 1){
                sub_level.Objects[near_id].CalculateASM(TexImg);
            }
			...
			if(!sub_level.Objects[near_id].merged){//if Neighbor not merged
                tf = 0.0;//added
     			for(int angle=0;angle<4;angle++){
     			    tf += fabs(sub_level.Objects[cur_id].ASM[angle]-sub_level.Objects[near_id].ASM[angle]);//Check abs function for double
     			}
     			if(tf<=passn.Texture){
...
int Slow_Mode::nthpass(Level& passn, ERS_Image& Img, Texture_Image& TexImg, float level_ID, MSEG_Init& mseg, MSEG_Param& Param, Level& sub_level, Level& super_level){
	 	...
	 	if(matches){
	 	    ...
	 	    for(int i=0; i<sub_level.Objects[best_id].Neighbors.size(); i++){
		        ...
		        if(!sub_level.Objects[near_id].priority_loaded){
	 	        ...
	 	        if(!sub_level.Objects[near_id].merged){//if Neighbor not merged
                    if((super_level.raster[D1(Img.Lines, Img.Columns, sub_level.Objects[near_id].Boundary[0].Line, sub_level.Objects[near_id].Boundary[0].ColumnStart)])==(super_level.raster[D1(Img.Lines, Img.Columns, sub_level.Objects[best_id].Boundary[0].Line, sub_level.Objects[best_id].Boundary[0].ColumnStart)])){
					    if (!sub_level.Objects[best_id].virtual_merged){//if neighbor merging heterogeneity not calculated
                            //Virtual merge into temp object
                        	tmp.clear();
                        	tmp.merge(sub_level.Objects[best_id], sub_level.Objects[near_id]);
...
int Slow_Mode::nthpass(Level& passn, ERS_Image& Img, Texture_Image& TexImg, MSEG_Init& mseg, Level& previous_pass){
    ...
    passn.Texture = previous_pass.Texture;//added
    //cout << "Texture parameter is " << passn.Texture << endl;
    ...
    while (!mseg.PriorityList.empty()){
		...
		//cout << "1" << endl;
		//If not calculated, calculate object's texture features //added
		if(previous_pass.Objects[cur_id].SDM.size() <= 1){
		    previous_pass.Objects[cur_id].BuildSDM(TexImg);
		}
		if(previous_pass.Objects[cur_id].ASM.size() <= 1){
		    previous_pass.Objects[cur_id].CalculateASM(TexImg);
		}
		//Update and Fix the neighbors for the previous level in order to call once bellow
		...
		for(int i=0; i<previous_pass.Objects[cur_id].Neighbors.size(); i++){
		    ...
		    //cout << "2" << endl;
		    //If not calculated, calculate object's texture features //added
            if(previous_pass.Objects[near_id].SDM.size() <= 1){
                previous_pass.Objects[near_id].BuildSDM(TexImg);
            }
            //cout << "was here" << endl;
            if(previous_pass.Objects[near_id].ASM.size() <= 1){
                previous_pass.Objects[near_id].CalculateASM(TexImg);
            }
            //cout << "was here..." << endl;
			...
			if(!previous_pass.Objects[near_id].merged){//if Neighbor not merged
				...
				//cout << "Neighbor not merged, inside if..." << endl;//Debug mode
     			tf = 0.0;//added
     			for(int angle=0;angle<4;angle++){
     			    tf += fabs(previous_pass.Objects[cur_id].ASM[angle]-previous_pass.Objects[near_id].ASM[angle]);//Check abs function for double
     			}
     			//cout << tf << endl;
     			if(tf<=passn.Texture){
                    if (!previous_pass.Objects[cur_id].virtual_merged){//if neighbor merging heterogeneity not calculated
...
int Slow_Mode::nthpass(Level& passn, ERS_Image& Img, Texture_Image& TexImg, MSEG_Init& mseg, Level& previous_pass, Level& super_level){
    ...
    passn.Compactness = previous_pass.Compactness;
    passn.Edge = previous_pass.Edge;
    passn.Texture = previous_pass.Texture;
    //cout << "Segmentation parameters: " << passn.ScaleParameter << " " << passn.Color
    //<< " " << passn.Compactness << endl;//Debug mode
